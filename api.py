...
def generate_followup(name: str, service: str, interest: str) -> str:
    resp = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a professional sales assistant. Write a short, friendly SMS under 120 characters. Include the customer's name. Be conversational, not corporate. End with a question that encourages a reply."
            },
            {
                "role": "user",
...
def generate_followup_sequence(name: str, service: str, interest: str):
    resp = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Create 3 SMS follow-up messages for a lead. Message 1: immediate. Message 2: 24-hour reminder. Message 3: final nudge. Keep each under 120 characters. Friendly tone."
            },
            {
                "role": "user",
                "content": f"Name: {name}\nService: {service}\nInterest: {interest}"
